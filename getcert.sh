...
firstcert () {
  ...
  if [ $(sudo docker ps | grep -c nginx) != "0" ]
  then
  ...
  -x509 -nodes -newkey rsa:1024 -days 1\
  -keyout "$virt_certp/privkey.pem" \
...
renewcert () {
  ...
  if [ $(sudo tail -n 50 /var/log/syslog | grep "The following certs have been renewed") ]
  then
  ...
  #Delete second log, empty ???
  #sudo rm $log_path/letsencrypt.log.*
}
